...
"""P2-native32 CONTROL — static-envelope Hyena on CIFAR-10 at native 32×32 resolution (no patchification).

Apples-to-apples baseline for ``envelope_warp_native32.py``.  Identical to
``baseline_hyena.py`` except ``PATCH_SIZE=1``, which gives a 32×32 = 1024-token
spatial grid instead of the 8×8 = 64-token grid used in the patched experiments.

The larger grid is the key hypothesis: at 8×8, the Gaussian envelope already
covers most of the grid and per-input shift/dilation has little room to be
meaningful.  At 32×32, the envelope covers a smaller fraction of the grid,
leaving more room for the mechanism to selectively shift or widen the kernel's
effective receptive field.

Architecture is otherwise identical to ``baseline_hyena.py`` so that any
accuracy difference between this run and ``envelope_warp_native32.py`` is
attributable entirely to the envelope-warp mechanism.
...
Run (local GPU, offline W&B)::
    ...
    python experiments/run.py \\
        --config examples/dynamic_deformable/baseline_hyena_native32.py \\
        experiment_dir=.runs/baseline_hyena_native32_seed42

Smoke test::

    python experiments/run.py \\
        --config examples/dynamic_deformable/baseline_hyena_native32.py \\
        experiment_dir=.runs/smoke_native32_base \\
        train.iterations=6 \\
...
HIDDEN_DIM = 192
NUM_BLOCKS = 6
PATCH_SIZE = 1  # no patchification: 32×32 = 1024 token grid (vs 8×8 = 64 in patched)
NUM_REGISTERS = 0  # no register-based conditioning in this static baseline
MLP_RATIO = 4
...
def get_config() -> ExperimentConfig:
    """Build the native-32 static baseline (control for the native-resolution P2 experiment)."""
    ...
    config.wandb = WandbConfig(
        ...
        tags=["dynamic-deformable", "cifar10", "p2-native32", "baseline"],
    )
