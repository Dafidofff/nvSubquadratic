...
"""P2-native32 — Dynamic Envelope Warping on CIFAR-10 at native 32×32 resolution (no patchification).

Minimal diff on top of ``baseline_hyena_native32.py``.  Identical except for the
``DynamicGaussianModulationND`` mask (register-based shift + dilation of the
Gaussian envelope), ``NUM_REGISTERS=4``, and the register-split adapter.

Hypothesis: at the 8×8 patch grid used in ``envelope_warp.py``, the Gaussian
envelope already covers most of the small grid and per-input shift/dilation adds
noise rather than useful inductive bias.  At 32×32, the kernel covers a smaller
fraction of the total grid, giving the dynamic mechanism room to selectively shift
or widen the effective receptive field per-input.

Diff vs. ``baseline_hyena_native32.py``:
  1. ``NUM_REGISTERS=4`` and register-split adapter (``grid_h=32``, ``register_write=True``).
  2. ``DynamicGaussianModulationND`` with ``cond_dim=HIDDEN_DIM`` replaces the static
     ``GaussianModulationND``.  Zero-init warp head → identity at step 0 (≡ baseline).
  3. ``register_pooling_cfg`` routes register tokens to ``conditioning=[B, C]``.
...
Run::
    ...
    python experiments/run.py \\
        --config examples/dynamic_deformable/envelope_warp_native32.py \\
        experiment_dir=.runs/envelope_warp_native32_seed42

Smoke test::

    python experiments/run.py \\
        --config examples/dynamic_deformable/envelope_warp_native32.py \\
        experiment_dir=.runs/smoke_native32_warp \\
        train.iterations=6 \\
...
HIDDEN_DIM = 192
NUM_BLOCKS = 6
PATCH_SIZE = 1  # native resolution: 32×32 = 1024 token grid
NUM_REGISTERS = 4
MLP_RATIO = 4
LAYER_SCALE_INIT = 1e-4
DROP_PATH_RATE = 0.05

# SIREN kernel (static — P2 only changes the envelope)
...
WARP_HEAD_HIDDEN_DIM = 32

# ── Training recipe ───────────────────────────────────────────────────────────
BATCH_SIZE = 64   # 128 OOMs on a 24 GB 3090 at 32×32 (1024 tokens); 64 fits comfortably
EPOCHS = 50
...
def get_config() -> ExperimentConfig:
    """Build the native-32 P2 envelope-warp config for CIFAR-10."""
    ...
    config.wandb = WandbConfig(
        ...
        tags=["dynamic-deformable", "cifar10", "p2-native32", "envelope-warp"],
    )
