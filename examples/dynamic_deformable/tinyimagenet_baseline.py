...
"""P0 baseline — static-conditioning Hyena ViT-5 classifier on TinyImageNet.

TinyImageNet port of ``baseline_hyena.py`` (CIFAR-10 P0). Same backbone and
training recipe; the only changes are the dataset (TinyImageNet, 200 classes,
native 64x64) and the patch size, so any accuracy gap vs the CIFAR-10 series is
attributable to task difficulty rather than architecture.

Goal: test whether the dynamic-deformable expressivity gap (P5 − P0) grows on a
harder classification task. This is the CONTROL; ``tinyimagenet_combined_best.py``
is the P5 (P1 spectral FiLM + P3 sparse mask) treatment.

Grid: ``PATCH_SIZE`` defaults to 8 → 64 // 8 = 8x8 = 64-token grid (same token
count as the CIFAR-10 runs, isolating task difficulty). Override ``PATCH_SIZE=4``
for the 16x16 = 256-token variant.

Run (ivi, online W&B)::

    conda activate nvsubq
    export PYTHONPATH=.
    python experiments/run.py --config examples/dynamic_deformable/tinyimagenet_baseline.py \\
        experiment_dir=runs/dynamic_deformable/tin_baseline_seed42 debug=false
"""
...
INPUT_CHANNELS = 3
IMAGE_SIZE = TINYIMAGENET_IMAGE_SIZE  # 64 — native resolution, no upsample
NUM_CLASSES = TINYIMAGENET_NUM_CLASSES  # 200

# ── Model (lightweight; identical to the CIFAR-10 P0 backbone) ────────────────
HIDDEN_DIM = 192
NUM_BLOCKS = 6
PATCH_SIZE = int(os.environ.get("PATCH_SIZE", "8"))  # 64 // 8 = 8×8 = 64 token grid
NUM_REGISTERS = 0  # P0 control: registers unused (see baseline_hyena.py note)
MLP_RATIO = 4
...
# ── Training recipe (identical to the CIFAR-10 series) ────────────────────────
BATCH_SIZE = 128
EPOCHS = 50
ITERS_PER_EPOCH = TINYIMAGENET_TRAIN_SIZE // BATCH_SIZE  # 100_000 // 128 = 781
WARMUP_EPOCHS = 2
LEARNING_RATE = 1e-3
WEIGHT_DECAY = 0.05
GRAD_CLIP = 1.0
...
def get_config() -> ExperimentConfig:
    """Build the P0 static-conditioning Hyena baseline for TinyImageNet."""
    config = ExperimentConfig()
    config.debug = True  # offline W&B by default; set debug=false for online logging
    config.seed = SEED
    ...
    config.wandb = WandbConfig(
        ...
        tags=["dynamic-deformable", "tiny-imagenet", "p0-baseline", f"patch{PATCH_SIZE}"],
    )
